...
use syn::{spanned::Spanned as _, Expr, ExprLit, Lit};
...
fn parse_attrs(attrs: &[syn::Attribute]) -> syn::Result<Attributes> {
    let attributes = attrs
        ...
        .try_fold(Attributes { bits: None }, |mut acc: Attributes, attr| {
            ...
            acc.bits = match &name_value.value {
                Expr::Lit(ExprLit {
                    lit: Lit::Int(lit), ..
...
fn generate_enum(input: syn::ItemEnum) -> syn::Result<TokenStream2> {
    ...
    let check_discriminants = variants.iter().map(|ident| {
        let span = ident.span();
        quote_spanned!(span =>
            impl ::scryer_modular_bitfield::private::checks::CheckDiscriminantInRange<[(); Self::#ident as usize]> for #enum_ident {
    ...
    let from_bytes_arms = variants.iter().map(|ident| {
        let span = ident.span();
        quote_spanned!(span=>
            __bitfield_binding if __bitfield_binding == Self::#ident as <Self as ::scryer_modular_bitfield::Specifier>::Bytes => {
    ...
    Ok(quote_spanned!(span=>
        ...
        impl ::scryer_modular_bitfield::Specifier for #enum_ident {
            const BITS: usize = #bits;
            type Bytes = <[(); #bits] as ::scryer_modular_bitfield::private::SpecifierBytes>::Bytes;
            type InOut = Self;

            #[inline]
            fn into_bytes(input: Self::InOut) -> ::core::result::Result<Self::Bytes, ::scryer_modular_bitfield::error::OutOfBounds> {
                ::core::result::Result::Ok(input as Self::Bytes)
            }

            #[inline]
            fn from_bytes(bytes: Self::Bytes) -> ::core::result::Result<Self::InOut, ::scryer_modular_bitfield::error::InvalidBitPattern<Self::Bytes>> {
                match bytes {
                    #( #from_bytes_arms ),*
                    invalid_bytes => {
                        ::core::result::Result::Err(
                            <::scryer_modular_bitfield::error::InvalidBitPattern<Self::Bytes>>::new(invalid_bytes)
                        )
                    }
                }
            }
